...
static int _response_with_empty_answer(const char *strWord, string&);
...
int Response::ResponseResult(const char *str_query_word, score_container &query_ans,
	...
	if(query_ans.empty()){
		return _response_with_empty_answer(str_query_word, response);
	}
	//打开原始数据文件
	//todo:mmap实现
	std::ifstream ifsData(file_path, ios::in | ios::binary);
	...
	while (pos_start < len) {
		...
		*pos_url_end = '\0';
		...
		os << "<a href=" << pos_url_start << "><strong>" << title_tmp<< "</strong></a>\n"
			<< "<br><div>\n";
		...
		os<<"\nceshi:\n"<<pos_url_start;
#endif
		_abstract_and_highlight(pos_title_end, words, idf, response);
...
//brief:输出到string
int Response::ResponseTop(const char *query_string, string& response) {
	//const char *local_address = getenv("HTTP_HOST");
	stringstream os;
	//头部
	...
	string tmp_str(query_string);
	replace(tmp_str.begin(),tmp_str.end(),kSEPARATE_CHAR,' ');
	os << "<tr style=\"text_align:center;vertical-align:middle;\">\n<td><span><form method=get action=" << kCGI_QUERY_PATH
		<< " name=got class=f>"
		"<input type=text name=word size=55 value=\"" << tmp_str << "\">\n"//注意这个“”“”是必不可少的
		"<input type=submit value=搜索>\n"
		"<input type=hidden name=start value=1>\n"//隐藏标签
...
//brief:输出到string
int Response::ResponseSelectPage(int number_result, int time_used, int page_no, const char *word_md5, string& response) {
	stringstream os;
	os << "<span>Dream9 为您找到约<b>" << number_result << "</b>篇文档,耗时<b>"
		<< time_used << "</b>毫秒</span>\n"
		"<table class=idx}><tr>";
	if (page_no > 1) {
		os << "<td><a href=" << kCGI_LEFT_PAGE_PATH << "?sessionid=" << word_md5 << "&page=" << page_no - 1 << ">上一页</a></td>";
	}
	else {
		os << "<td>上一页</td>";
	}
	...
	//网页尾端内容
	os << "<br><hr><br><p style=text-align:center;>&copy Copyleft Dream9</p><br><br>\n"
		"</body>\n</html>";
	response.append(os.str());
	return 0;
}
...
//brief:输出到string
int Response::_abstract_and_highlight(const char *str, vector<string> &words, vector<float> &idf, string& response) {
	stringstream os;
	os << "<span style=\"color:blue; font - weight:bold\"> 摘要 </span>\n";
	//
	Document doc_parser;
	doc_parser.RefineWithSeparate(str);
	//
	...
	//及其特殊的情况，也就是标题党。。。仅在标题中出现了这个关键字，之后再也没有出现。。。
	if (max_weight_position == -1) {
		os << "<span>This page is <strong>just a title</strong> ! Kill him!</span>";
		return -1;
	}
	//动态摘要生成
	//根据长度限制做必要的扩展收缩，并高亮显示
...
//brief:输出到string
int _response_with_empty_answer(const char *strWord, string& response){
	response.append("<p>surprise!!空空如也</p>\n");
	return 0;
}
